...
public class RoundProgressBar extends View {
    ...
    public static int DEFAULT_STROKE_WIDTH = RxUtils.dp2px(40);
    RoundProgressBarTextGenerator mRoundProgressBarTextGenerator;
    ...
    public void setup(Context context, AttributeSet attrs) {
        ...
        mType = array.getInt(R.styleable.RoundProgressBar_pro_type, TYPE_RECT);
        mProgressColor = array.getColor(R.styleable.RoundProgressBar_pro_progress_color, DEFAULT_PROGRESS_COLOR);
        mBackgroundColor = array.getColor(R.styleable.RoundProgressBar_pro_background_color, DEFAULT_BACKGROUND_COLOR);

        mMaxValue = array.getInt(R.styleable.RoundProgressBar_pro_max_value, 100);
        mValue = array.getInt(R.styleable.RoundProgressBar_pro_value, 0);

        boolean isRoundCap = array.getBoolean(R.styleable.RoundProgressBar_pro_stroke_round_cap, false);

        int textSize = DEFAULT_TEXT_SIZE;
        if (array.hasValue(R.styleable.RoundProgressBar_android_textSize)) {
            textSize = array.getDimensionPixelSize(R.styleable.RoundProgressBar_android_textSize, DEFAULT_TEXT_SIZE);
        }
        int textColor = DEFAULT_TEXT_COLOR;
        if (array.hasValue(R.styleable.RoundProgressBar_android_textColor)) {
            textColor = array.getColor(R.styleable.RoundProgressBar_android_textColor, DEFAULT_TEXT_COLOR);
        }

        if (mType == TYPE_CIRCLE) {
            mStrokeWidth = array.getDimensionPixelSize(R.styleable.RoundProgressBar_pro_stroke_width, DEFAULT_STROKE_WIDTH);
        }
        array.recycle();
     ...
     * @see #setRoundProgressBarTextGenerator(RoundProgressBarTextGenerator)
     */
    ...
    /**
     * 通过 {@link RoundProgressBarTextGenerator} 设置进度文案
     */
    public void setRoundProgressBarTextGenerator(RoundProgressBarTextGenerator RoundProgressBarTextGenerator) {
        mRoundProgressBarTextGenerator = RoundProgressBarTextGenerator;
    }

    public RoundProgressBarTextGenerator getRoundProgressBarTextGenerator() {
        return mRoundProgressBarTextGenerator;
    }

    @Override
    protected void onDraw(Canvas canvas) {
        if (mRoundProgressBarTextGenerator != null) {
            mText = mRoundProgressBarTextGenerator.generateText(this, mValue, mMaxValue);
        }
        if (mType == TYPE_RECT) {
            drawRect(canvas);
        } else {
    ...
    public void setProBackgroundColor(@ColorInt int backgroundColor) {
    ...
    public interface RoundProgressBarTextGenerator {
        /**
         * 设置进度文案, {@link RoundProgressBar} 会在进度更新时调用该方法获取要显示的文案
         *
         * @param value    当前进度值
         * @param maxValue 最大进度值
         * @return 进度文案
         */
        String generateText(RoundProgressBar progressBar, int value, int maxValue);
    }
}
